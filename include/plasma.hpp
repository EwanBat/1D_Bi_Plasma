...
public:
    ...
    Eigen::VectorXd m_n_i1;      // Ion density perturbation [m^-3]
    Eigen::VectorXd m_u_i1;      // Ion velocity [m/s]
    Eigen::VectorXd m_n_e1;      // Electron density perturbation [m^-3]
    Eigen::VectorXd m_u_e1;      // Electron velocity [m/s]
    Eigen::VectorXd m_E;         // Electric field [V/m]
    Eigen::VectorXd m_E0;
    
    // Grid and time stepping parameters
    int m_Nx;                    // Number of spatial points
    double m_dx;                 // Spatial step size [m]
    double m_dt;                 // Time step size [s] (adaptive via CFL)
    double m_cfl_factor;         // CFL safety factor (typically 0.5)
    
    // Physical parameters for both species
    PlasmaParams m_params;
...
public:
    ...
    PlasmaSystem(int nx, double dx_val, const PlasmaParams& p, const double cfl = 0.5) 
        : m_Nx(nx), m_dx(dx_val), m_params(p), m_cfl_factor(cfl) {
        // Initialize state vectors to zero
        m_n_i1.resize(m_Nx+1); m_n_i1.setZero();
        m_u_i1.resize(m_Nx+1); m_u_i1.setZero();
        m_n_e1.resize(m_Nx+1); m_n_e1.setZero();
        m_u_e1.resize(m_Nx+1); m_u_e1.setZero();
        m_E.resize(m_Nx+1); m_E.setZero();
        m_E0.resize(m_Nx+1); m_E0.setZero();
        
        // Initialize temporary storage vectors
        m_Phi.resize(m_Nx+1); m_Phi_star.resize(m_Nx+1);
        m_E_star.resize(m_Nx+1);

        m_n_e_star.resize(m_Nx+1); m_n_i_star.resize(m_Nx+1);
        m_u_e_star.resize(m_Nx+1); m_u_i_star.resize(m_Nx+1);

        m_F_e_n.resize(m_Nx+1); m_F_i_n.resize(m_Nx+1);
        m_F_i_u.resize(m_Nx+1); m_F_e_u.resize(m_Nx+1);
...
private:
    ...
    void compute_time_step() {
        ...
        for (int i = 0; i < m_Nx+1; ++i) {
            double speed_i = std::abs(m_u_i1(i)) + m_params.cs_i;
            double speed_e = std::abs(m_u_e1(i)) + m_params.cs_e;
            max_speed = std::max({max_speed, speed_i, speed_e});
        }
        m_dt = m_cfl_factor * m_dx / max_speed;
    }
     ...
     * @param u_i Ion velocity [m/s]
     */
    void apply_boundary_conditions(Eigen::VectorXd& n_e, Eigen::VectorXd& n_i,
...
public:
    ...
    void init_time_files(const std::vector<double>& x_grid, const std::string& prefix = "../data/") {
        ...
        m_file_t.open(prefix + "time.txt");
        m_file_n_i1.open(prefix + "n_i1_time.txt");
        m_file_u_i1.open(prefix + "u_i1_time.txt");
        m_file_n_e1.open(prefix + "n_e1_time.txt");
        m_file_u_e1.open(prefix + "u_e1_time.txt");
        m_file_E.open(prefix + "E_time.txt");
        
        if (!m_file_t.is_open() || !m_file_n_i1.is_open() || !m_file_u_i1.is_open() || 
            !m_file_n_e1.is_open() || !m_file_u_e1.is_open() || !m_file_E.is_open()) {
    ...
    void append_time_data(double& t) {
        ...
        for (int i = 0; i < m_Nx+1; ++i) {
            m_file_n_i1 << m_n_i1(i);
            m_file_u_i1 << m_u_i1(i);
            m_file_n_e1 << m_n_e1(i);
            m_file_u_e1 << m_u_e1(i);
            m_file_E << m_E(i);
            
            if (i < m_Nx) {  // Add space separator between values
                m_file_n_i1 << " ";
                m_file_u_i1 << " ";
                m_file_n_e1 << " ";
                m_file_u_e1 << " ";
    ...
    void close_time_files() {
        if (m_file_t.is_open()) m_file_t.close();
        if (m_file_n_i1.is_open()) m_file_n_i1.close();
        if (m_file_u_i1.is_open()) m_file_u_i1.close();
        if (m_file_n_e1.is_open()) m_file_n_e1.close();
        if (m_file_u_e1.is_open()) m_file_u_e1.close();
        if (m_file_E.is_open()) m_file_E.close();
    }
    ...
    /**
    * @brief Set a constant perturbation in the electric field
    * 
    * @param E0 Amplitude of the electric field perturbation [V/m]
    */
    void set_constant_electric_field(Eigen::VectorXd E0) {
