#include <iostream>
#include "../include/Eigen/Dense"
#include "../include/Eigen/src/Core/Matrix.h"
#include <complex>
#include <string>
#include <vector>

#include "../include/field.hpp"
#include "../include/plasma.hpp"
#include "../include/bi_system.hpp"

const double ne0 = 1e8; // Density in m^-3
const double ni0 = 1e8; // Equilibrium density in m^-3
const double T_e = 1e3; // Electron temperature in K
const double T_i = 1e3; // Ion temperature in K
const double qe = -consts::e; // Electron charge in C
const double qi = consts::e; // Ion charge in C
const double me = consts::me; // Electron mass in kg
const double mi = consts::mp; // Ion mass in kg (proton mass)

void pert_gaussian(bi_system &system, std::vector<double> &t_grid, std::vector<double> &x_grid, std::vector<double> &kx_grid, int Nx, int Nkx, int Nt){

    // Real-space perturbations must be sized by Nx (space) x Nt (time)
    Eigen::MatrixXd n1e_x(Nx, Nt); n1e_x.setZero();
    Eigen::MatrixXd n1i_x(Nx, Nt); n1i_x.setZero();
    Eigen::MatrixXcd n1e_k(Nkx, Nt); n1e_k.setZero();
    Eigen::MatrixXcd n1i_k(Nkx, Nt); n1i_k.setZero();
    double x0 = 0; // Center position of the perturbation [m]
    double sigma_x = 1; // Width of the perturbation [m]
    double amplitude = 1e-3; // Amplitude of the density perturbation

    for (int i = 0; i < Nx; ++i) {
        double x = x_grid[i];
        n1e_x(i, 0) = ne0 * amplitude * std::exp(-0.5 * std::pow((x - x0) / sigma_x, 2));
        n1i_x(i, 0) = ni0 * amplitude * std::exp(-0.5 * std::pow((x - x0) / sigma_x, 2));
    }

    fourier_transform_initial(n1e_x, n1e_k, t_grid, kx_grid, x_grid);
    fourier_transform_initial(n1i_x, n1i_k, t_grid, kx_grid, x_grid);

    std::vector<Eigen::MatrixXcd> U1e_k(2); U1e_k[0].resize(Nkx, Nt); U1e_k[0].setZero(); U1e_k[1].resize(Nkx, Nt); U1e_k[1].setZero();
    std::vector<Eigen::MatrixXcd> U1i_k(2); U1i_k[0].resize(Nkx, Nt); U1i_k[0].setZero(); U1i_k[1].resize(Nkx, Nt); U1i_k[1].setZero();
    system.m_electron.initial_perturbation(std::move(n1e_k), std::move(U1e_k));
    system.m_ion.initial_perturbation(std::move(n1i_k), std::move(U1i_k));
}

void pert_magnetic(bi_system &system, std::vector<double> &kx_grid, const int &Nkx, const int &Nt){
    Eigen::MatrixXcd E1_k(Nkx, Nt); E1_k.setZero();
    Eigen::MatrixXcd B1_k(Nkx, Nt); B1_k.setZero();
    double kx0 = 5e-2; // Central wave number of the perturbation [m^-1]
    double sigma_kx = 1e-3; // Width of the perturbation [m^-1]
    double amplitude = 1e-8; // Amplitude of the magnetic field perturbation [T]

    for (int i = 0; i < Nkx; ++i) {
        double kx = kx_grid[i];
        B1_k(i, 0) = std::complex<double>(0, amplitude);
    }

    system.m_field.initial_perturbation(std::move(E1_k), std::move(B1_k));
}

int main() {
    double t0 = 0; // Initial time in s
    double tf = 1e-5; // Final time in s
    double dt = 1e-8; // Time step in s
    const int Nt = ((tf - t0) / dt) + 1; // Number of time steps
    std::vector<double> t_grid(Nt);
    for (int i = 0; i < Nt; ++i) {t_grid[i] = t0 + i * dt;}
    std::cout << "Number of time steps: " << Nt << std::endl;

    double kxi = 1e-1; // Minimum wave number in m^-1
    double kxf = 10; // Maximum wave number in m^-1
    double dkx = 5e-2; // Wave number step in m^-1
    const int Nkx = ((kxf - kxi) / dkx) + 1; // Number of wave number steps
    std::vector<double> kx_grid(Nkx);
    for (int i = 0; i < Nkx; ++i) {kx_grid[i] = kxi + i * dkx;}
    std::cout << "Number of kx steps: " << Nkx << std::endl;

    double x0 = -2.0 * M_PI / kxi; // Minimum position in m
    double xf = 2.0 * M_PI / kxi; // Maximum position in m
    double dx = 2.0 * M_PI / (dkx * Nkx); // Position step in m
    const int Nx = ((xf - x0) / dx) + 1; // Number of position steps
    std::vector<double> x_grid(Nx);
    for (int i = 0; i < Nx; ++i) {x_grid[i] = x0 + i * dx;}
    std::cout << "Number of x steps: " << Nx << std::endl;

    // Initialize plasma species
    bi_system system;
    system.m_electron.initial_constant(ne0, qe, me, T_e);
    system.m_ion.initial_constant(ni0, qi, mi, T_i);
    system.setup_system(Nkx, Nt);
    system.print_parameter();

    // Initial perturbations in kx-space
    pert_gaussian(system, t_grid, x_grid, kx_grid, Nx, Nkx, Nt);
    // pert_magnetic(system, kx_grid, Nkx, Nt);

    // Iterate over time and wave numbers and transform back to real space
    system.iteration(t0, tf, dt, kxi, kxf, dkx);
    system.save_datas(t_grid, kx_grid, x_grid);
    system.clear_system();

    std::string cmd = "python3 ../src/plot_time.py";
    int result = std::system(cmd.c_str());
    if (result != 0) {
        std::cerr << "Erreur lors de l'affichage Python.\n";
    }

    std::string cmd2 = "python3 ../src/plot_pulsation.py";
    int result2 = std::system(cmd2.c_str());
    if (result2 != 0) {
        std::cerr << "Erreur lors de l'affichage Python.\n";
    }
    return 0;
}